    ...
    Medical_History.findAll({where: {client_id: id}, order: [['date', 'DESC']]})
        .then(medical_history => {
            if (!medical_history) {
                return res.status(404)
                    .setHeader('content-type', 'application/json')
                    .send({error: `Medical history record not found for client with id: ${id}`});
            }
...
router.delete('/clients/medical-histories/delete/:id', (req, res) => {
    ...
    return db.transaction(async (t) => {
        ...
        if (!medical_history) {
            return res.status(404)
                .setHeader('content-type', 'application/json')
                .send({error: `Medical history record not found!`});
        }
        ...
        return medical_history.destroy({transaction: t})
            .then(medical_history => {
                res.status(200)
                    .setHeader('content-type', 'application/json')
                    .send({message: `Medical history record deleted!`, medical_history: medical_history});
            })
    })
        .catch(error => {
            return res.status(500)
